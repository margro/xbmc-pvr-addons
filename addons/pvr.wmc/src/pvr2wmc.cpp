...
{
	CStdString request;
	request.Format("GetServiceStatus|%s|%s", g_clientVersion, g_clientOS);
...
PVR_ERROR Pvr2Wmc::AddTimer(const PVR_TIMER &xTmr)  
{
	...
	else 
	{
		...
		PVR->TriggerTimerUpdate();					// tell xbmc to update timer display
		XBMC->Log(LOG_DEBUG, "recording added for timer '%s', with rec state %s", xTmr.strTitle, results[0].c_str());

		if (results.size() > 1 /*&& results[1] == "recordingNow"*/)		// if there is extra results sent from server...
		{
...
int Pvr2Wmc::ReadLiveStream(unsigned char *pBuffer, unsigned int iBufferSize)
{
	...
	if (lpNumberOfBytesRead != iBufferSize)
