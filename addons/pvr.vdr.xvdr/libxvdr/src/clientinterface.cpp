...
void ClientInterface::Log(LOGLEVEL level, const char* text, ...)
{
  ...
  vsnprintf(m_msg, MAX_MESSAGE_SIZE, text, ap);
  va_end(ap);
...
void ClientInterface::Notification(LOGLEVEL level, const std::string& text)
{
  Notification(level, text.c_str());
}

void ClientInterface::Notification(LOGLEVEL level, const char* text, ...)
{
  MutexLock lock(&m_msgmutex);
